...
public:
    SparrowRpcClient() : hStdInWrite(NULL), hStdOutRead(NULL), 
    ...
    // 启动 Rust 程序
    bool start(const std::string& exePath, const std::string& inputJson) {
        ...
        if (!CreatePipe(&hStdErrRead, &hStdErrWrite, &sa, 0)) {
            std::cerr << "Failed to create stderr pipe\n";
            return false;
        }
        ...
        // 构建命令行
        std::string cmdLine = exePath + " --output-mode stdout -";
        ...
        // 关闭 stdin 写端，通知 Rust 输入结束（关键！否则 Rust 会一直阻塞等待 EOF）
        CloseHandle(hStdInWrite);
        hStdInWrite = NULL;
        
        // 启动读取线程
        running = true;
        readerThread = std::thread(&SparrowRpcClient::readOutput, this);
    ...
    void requestStop() {
        // 创建控制文件通知 Rust 停止
        std::string controlFile = "stop_signal.txt";
